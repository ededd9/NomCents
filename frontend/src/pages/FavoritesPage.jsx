...
const FavoritesPage = () => {
  ...
  const updateGroceryListBackend = async (newGroceryList) => { // <-- Added function
    if (!isLoggedIn || !user) return;
    try {
      console.log("Updating grocery list from Favorites page for email:", user.email);
      const response = await fetch(`${BACKEND_API_URL}/user`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  const addToGroceryList = (productToAdd) => {
    ...
    setPopupMessage(`${productToAdd.name || productToAdd.description} added to cart!`);
